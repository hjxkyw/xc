...
confere 'atribuicao: op, e alvo com trailer vira texto', {
  my @c = corpo("  n += 1\n  oObj:nX := 2");
  @c[0].op eq '+=' && @c[0].alvo ~~ Nome && @c[1].alvo ~~ Opaca
};
confere 'chamada como comando: simples vira Chamada, com metodo vira texto', {
  my @c = corpo("  conout(\"a\", 1)\n  oDlg:Activate()");
  @c[0].chamada ~~ Chamada && @c[0].chamada.args == 2 && @c[1].chamada ~~ Opaca
};
